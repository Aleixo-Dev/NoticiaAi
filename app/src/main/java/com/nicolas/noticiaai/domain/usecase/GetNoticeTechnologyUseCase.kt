...
class GetNoticeTechnologyUseCase @Inject constructor(
    private val repository: NoticeRepository
) {
    suspend operator fun invoke(): Resource<List<NoticeUiDomain>> {
        return try {
            val listNoticeTechnology = repository.fetchNoticeTechnology()
            return if (listNoticeTechnology.isNotEmpty()) {
                Resource.Success(listNoticeTechnology)
            } else {
